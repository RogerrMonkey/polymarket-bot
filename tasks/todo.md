...
- Lesson: auth keys stayed empty in `.env` this session despite user instruction to assume filled — `_normalize_*` helpers are now in place so the unblock is a pure config change (no code edits needed) whenever MetaMask keys are pasted in.
- Pending: Fill POLYMARKET_PRIVATE_KEY + POLYMARKET_FUNDER_ADDRESS in .env (MetaMask keys) to unlock wallet/auth checks.
- Pending: Accumulate >=14 distinct paper-analysis days (currently 4/14) — scheduler now registered, days should accrue automatically provided WARP is active at 03:00 UTC.
